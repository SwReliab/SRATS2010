...
double txvmin_pdf(double t, double shape, double scale) {
  double y = exp((t+scale)/shape);
  double y0 = exp(scale/shape);
  return y*exp(-y)/shape/exp(-y0);
}
	
double txvmin_cdf(double t, double shape, double scale) {
  double y = exp((t+scale)/shape);
  double y0 = exp(scale/shape);
  return 1.0-exp(-y)/exp(-y0);
}

double txvmin_mvf(double t, double omega, double shape, double scale) {
  return omega * txvmin_cdf(t, shape, scale);
}

double txvmin_cdf_v(double t, int npara, double *para) {
	return txvmin_cdf(t, para[1], para[2]);
}

void txvmin_inverse_mvf
(double *value, double *time, int *npara, double *para) {
  *time = findQuantile(*value/para[0], txvmin_cdf_v, *npara, para);
}
...
(int *dsize, double *time, double *lambda, int *npara, double *para) {
  ...
  for (i=0; i<*dsize; i++) {
    t += time[i];
    lambda[i] = omega * txvmin_pdf(t, shape, scale);
  }
}

double txvmin_creli(double x, double s, 
		   double omega, double shape, double scale,
		   double sval, double ffp) {
  double tmp;
  tmp = txvmin_mvf(x + s, omega, shape, scale) - sval;
...
(double *stime, double *r, double *ss, int *npara, double *para) {
  ...
  sval = txvmin_mvf(*stime, omega, shape, scale);
  ffp = exp(-(omega - sval));

  rev = sval - log(*r + (1.0-*r)*ffp);
  txvmin_inverse_mvf(&rev, &b, npara, para);
  b = b - *stime;
  
  // trapezoidal inte
  h = b - a;
  t = h * (txvmin_creli(a, *stime, omega, shape, scale, sval, ffp)
	   + txvmin_creli(b, *stime, omega, shape, scale, sval, ffp)) / 2.0;
  ...
  for (k = 1; k <= MTTF_MAXITE; k++) {
    ...
    s = 0.0;
    for (i=1; i<=n-1; i+=2) {
      s += txvmin_creli(a + i * h, *stime, omega, shape, scale, sval, ffp);
    }
    tn = t/2.0 + h * s;
...
(int *dsize, double *time, double *mean, int *npara, double *para) {
  ...
  for (i=0; i<*dsize; i++) {
    t += time[i];
    mean[i] = txvmin_mvf(t, omega, shape, scale);
  }
}
...
(int *dsize, double *time, double *num, int *type, 
 int *npara, double *para, double *pdiff, double *retllf, double *total) {
  ...
  llf = 0.0;
  g00 = exp(-y0);
  g01 = exp(-scale/shape)*((1.0 + y0)*exp(-y0));
  g02 = exp(-y0) * (1.0 + y0 * log(y0));
  t = -time[0];
  x = num[0];
  y = exp(-(t-scale)/shape);
  g10 = g00;
  ...
  g12 = g02;
  g20 = exp(-y);
  g21 = exp(-scale/shape)*((1.0 + y)*exp(-y));
  ...
  if (type[0] == 1) {
    ...
    llf += log(txvmin_pdf(-t, shape, scale));
  }
  for (j=1; j<*dsize; j++) {
    x = num[j];
    if (time[j] != 0.0) {
